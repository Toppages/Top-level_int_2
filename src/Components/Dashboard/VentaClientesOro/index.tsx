...
function VentaClientesOro({ userHandle }: VentaAdminClientesProps) {
        ...
        const fetchSales = async () => {
            ...
            try {
                ...
                response.data.forEach(sale => {
                    const handle = sale.originalVendedorHandle || sale.user?.handle;
                    if (handle) handlesSet.add(handle);
                });

                const userOptions = Array.from(handlesSet).map(handle => ({
                    value: handle,
                    label: handle,
                }));
    ...
    useEffect(() => {
        let filtered = sales;
        // ✅ En el useEffect de filtrado por usuarios y fechas
        if (selectedUsers.length > 0 && !selectedUsers.includes('all')) {
            filtered = filtered.filter(sale => {
                const handle = sale.originalVendedorHandle || sale.user?.handle;
                return selectedUsers.includes(handle ?? '');
            });
        }
                ...
                {(selectedUsers.length === 0 || (selectedUsers.length > 1 && !selectedUsers.includes('all'))) && (
                    <>
                        ...
                        {users.slice(1).map(user => {
                            const ventasUsuario = filteredSales.filter(sale => {
                                const handle = sale.originalVendedorHandle || sale.user?.handle;
...
export default VentaClientesOro;